...
const categories = [
  ...
  { name: 'Hombre', description: 'Prendas con actitud', color: 'from-slate-400 to-slate-700' },
  { name: 'Accesorios', description: 'Complementos que enamoran', color: 'from-amber-300 to-amber-500' },
  { name: 'Calzado', description: 'Estilo para cada paso', color: 'from-stone-300 to-stone-600' },
]
...
const products = [
  {
    ...
    price: '$19.900',
    badge: 'Nuevo',
    description: 'Cálida, moderna y con corte relajado.',
  },
  {
    name: 'Vestido Midi Burdeos',
    price: '$22.500',
    badge: 'Oferta',
    description: 'Perfecto para eventos con estilo.',
  },
  {
    name: 'Bota Chelsea Cuero',
    price: '$24.800',
    badge: 'Nuevo',
    description: 'Clásicas y resistentes para todo el día.',
  },
  {
    name: 'Mochila Minimalista',
    price: '$11.990',
    badge: 'Oferta',
    description: 'Un must para tu look diario.',
  },
]
      ...
      <header className="border-b border-slate-200 bg-slate-950 text-white">
        ...
        <div className="mx-auto flex max-w-7xl items-center justify-between gap-4 px-4 py-5 sm:px-6">
          <div className="flex items-center gap-2 text-lg font-semibold uppercase tracking-[0.3em]">
            <span className="inline-flex h-10 w-10 items-center justify-center rounded-2xl bg-white text-slate-950">F</span>
            Fashion Store
          </div>
        ...
        <section className="relative overflow-hidden bg-slate-950 text-white">
          ...
          <div className="relative mx-auto flex max-w-7xl flex-col gap-10 px-4 py-24 sm:px-6 lg:flex-row lg:items-center lg:justify-between">
            <div className="max-w-2xl space-y-6">
              <p className="inline-flex rounded-full border border-white/20 bg-white/10 px-4 py-1 text-sm uppercase tracking-[0.3em] text-white/80">Colección Invierno</p>
              <h1 className="text-4xl font-semibold leading-tight sm:text-5xl lg:text-6xl">Nueva Colección Invierno 2026</h1>
              <p className="max-w-xl text-base text-slate-200 sm:text-lg">Descubrí las últimas tendencias con hasta 40% OFF en looks pensados para cada ocasión.</p>
              <div className="flex flex-col gap-4 sm:flex-row sm:items-center">
                <div className="flex flex-wrap gap-4">
  {/* Primario: fondo blanco, texto negro */}
  <Button className="bg-white text-black hover:bg-slate-200 font-semibold px-6 py-3">
    Comprar ahora
  </Button>
  
  {/* Secundario: transparente con borde blanco, texto blanco */}
  <Button variant="outline" className="border-2 border-white text-white bg-transparent hover:bg-white hover:text-black font-semibold px-6 py-3">
    Ver novedades
            ...
            <div className="relative h-[420px] w-full overflow-hidden rounded-[2rem] border border-white/10 bg-gradient-to-br from-slate-900 via-slate-950 to-slate-800 shadow-2xl shadow-slate-950/20 lg:h-[580px]">
              <div className="absolute inset-0 bg-[url('https://images.unsplash.com/photo-1512436991641-6745cdb1723f?auto=format&fit=crop&w=1200&q=80')] bg-cover bg-center opacity-80" />
              ...
              <div className="absolute bottom-8 left-8 rounded-3xl border border-white/10 bg-white/10 p-6 backdrop-blur-sm">
                <p className="text-sm uppercase tracking-[0.3em] text-white/70">Oferta destcada</p>
                <h2 className="mt-2 text-2xl font-semibold text-white">Todo lo que necesitás para este invierno</h2>
                <p className="mt-1 text-sm text-slate-200">Looks premium con envío rápido y devolución simple.</p>
              </div>
            </div>
          </div>
        </section>
        ...
        <section id="categorias" className="mx-auto max-w-7xl px-4 py-16 sm:px-6">
          <div className="mb-10 text-center">
            ...
            <h2 className="mt-3 text-3xl font-semibold text-slate-950 sm:text-4xl">Encontrá tu estilo</h2>
          </div>
          ...
          <div className="mx-auto max-w-7xl px-4 sm:px-6">
            ...
            <div className="grid gap-6 sm:grid-cols-2 lg:grid-cols-4">
              {products.map((product) => (
                <article key={product.name} className="group overflow-hidden rounded-[2rem] border border-slate-200 bg-white p-5 shadow-sm transition hover:-translate-y-1 hover:shadow-xl">
                  <div className="relative mb-5 h-52 overflow-hidden rounded-[1.75rem] bg-slate-100">
                    <div className="absolute inset-0 bg-[url('https://images.unsplash.com/photo-1521334884684-d80222895322?auto=format&fit=crop&w=900&q=80')] bg-cover bg-center opacity-90 transition group-hover:scale-105" />
                    <div className="absolute inset-0 bg-gradient-to-t from-slate-950/80 via-slate-950/10 to-transparent" />
                    <Badge className="absolute left-4 top-4 bg-white/90 text-slate-950">{product.badge}</Badge>
                  </div>
                  <div className="space-y-3">
                    <h3 className="text-lg font-semibold text-slate-950">{product.name}</h3>
                    <p className="text-sm text-slate-500">{product.description}</p>
                    <div className="flex items-center justify-between gap-3">
                      <p className="text-xl font-semibold text-slate-950">{product.price}</p>
